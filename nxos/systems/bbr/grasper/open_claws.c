		...
		ActuatorState behavior_actuations[], ActuatorState availableStates[])
{
		...
		if (*claw_movement==CLAW_STOPPED)
		{
			...
			if(touched->reading.touched==FALSE && *claw_closed==TRUE)
			{

				*claw_movement=CLAW_OPENING;
				// start opening claw
				copyActuatorState(BBR_OPEN_CLAWS, behavior_actuations, availableStates, BBR_CLAWS_UNLOCK);
			}
			else
			{
				// Not Claw Opening Movement, deactivate Behavior and exit
				copyActuatorState(BBR_OPEN_CLAWS, behavior_actuations, availableStates, BBR_BEHAVIOR_INHIBITED);
			}


		}
		// if claw is opening
		else if (*claw_movement==CLAW_OPENING)
		{
			// if claw not moving yet
			if (has_claw_started_moving(actuators, current_state)==FALSE)
			{
				copyActuatorState(BBR_OPEN_CLAWS, behavior_actuations, availableStates, BBR_CLAWS_OPEN);
			...
			else
			{
				...
				if (has_claw_fully_stopped(claw_timestamp, claw_oldtstamp, claw_position, claw_oldpos)==TRUE)
				{
					...
					*claw_closed=FALSE;				// update claw closed status
					*claw_movement=CLAW_STOPPED;	// update claw movement status
					copyActuatorState(BBR_OPEN_CLAWS, behavior_actuations, availableStates, BBR_CLAWS_LOCK);
